...
public class Assignments {
    ...
    private final static HashMap<String, Random> generators = new HashMap<>();

    private static Random getGenerator(String user) {
        Random generator;
        if (!generators.containsKey(user)) {
            generator = new NotSoRandom();
            generators.put(user, generator);
        }
        else {
            generator = generators.get(user);
        }
        
        return generator;
    }
    
    public static Assignment random(String user) {
        int idx = getGenerator(user).nextInt(all.length);
        return all[idx];
    }
